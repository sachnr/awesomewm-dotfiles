...
local widget = wibox.widget({
	{
		...
		markup = string.format('<span foreground="%s"> 󱩌 </span>', beautiful.accent),
...
widget_boxed:connect_signal("button::press", function(_, _, _, button)
	if button == 1 then
		awesome.emit_signal("redshift::toggle")
	end
end)
...
awesome.connect_signal("redshift::toggle", function()
	if redshift_on then
		...
		awful.spawn.easy_async("redshift -l 0:0 -t 4200:4200")
...
awesome.connect_signal("redshift::off", function()
	awful.spawn.easy_async_with_shell(
		...
		function()
			...
			widget
				:get_children_by_id("text")[1]
				:set_markup(string.format('<span foreground="%s"> 󱩌 </span>', beautiful.accent))
		end
	)
end)
